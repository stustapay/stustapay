...
class BaseReportlabInvoiceRenderer(BaseInvoiceRenderer):
    ...
    def _get_stylesheet(self):
        ...
        stylesheet.add(
            ParagraphStyle(name="BoldRight", fontName=self.font_bold, fontSize=10, leading=12, alignment=TA_RIGHT)
        )
        stylesheet.add(
            ParagraphStyle(
        ...
        stylesheet.add(
            ParagraphStyle(name="NormalRight", fontName=self.font_regular, fontSize=10, leading=12, alignment=TA_RIGHT)
        )
        stylesheet.add(
            ParagraphStyle(
                name="BoldInverseCenter",
                fontName=self.font_bold,
                fontSize=10,
                leading=12,
        ...
        stylesheet.add(
            ParagraphStyle(
                name="FineprintRight", fontName=self.font_regular, fontSize=8, leading=10, alignment=TA_RIGHT
            )
        )
        return stylesheet
    ...
    def _register_fonts(self):
        ...
        pdfmetrics.registerFontFamily(
            "OpenSans", normal="OpenSans", bold="OpenSansBd", italic="OpenSansIt", boldItalic="OpenSansBI"
        )
    ...
    def _clean_text(self, text, tags=None):
        return self._normalize(
            bleach.clean(text, tags=set(tags) if tags else set())
            .strip()
            .replace("<br>", "<br />")
            .replace("\n", "<br />\n")
        )
...
class ClassicInvoiceRenderer(BaseReportlabInvoiceRenderer):
    identifier = "classic"
    verbose_name = "Classic renderer (pretix 1.0)"
    ...
    def _draw_invoice_to(self, canvas):
        p = Paragraph(self._clean_text(self.invoice.address_invoice_to), style=self.stylesheet["Normal"])
    ...
    def _draw_invoice_from(self, canvas):
        p = Paragraph(self._clean_text(self.invoice.full_invoice_from), style=self.stylesheet["InvoiceFrom"])
    ...
    def _draw_invoice_from_label(self, canvas):
        ...
        textobject.textLine(self._normalize(self._upper("Invoice from")))
        canvas.drawText(textobject)

    def _draw_invoice_to_label(self, canvas):
        textobject = canvas.beginText(25 * mm, (297 - 50) * mm)
        textobject.setFont(self.font_bold, 8)
        textobject.textLine(self._normalize(self._upper("Invoice to")))
        canvas.drawText(textobject)
    ...
    def _draw_metadata(self, canvas):
        ...
        textobject.setFont(self.font_bold, 8)
        textobject.textLine(self._normalize(self._upper("Order code")))
        ...
        if self.invoice.is_cancellation:
            textobject.textLine(self._normalize(self._upper("Cancellation number")))
            textobject.moveCursor(0, 5)
            textobject.setFont(self.font_regular, 10)
            textobject.textLine(self._normalize(self.invoice.number))
            textobject.moveCursor(0, 5)
            textobject.setFont(self.font_bold, 8)
            textobject.textLine(self._normalize(self._upper("Original invoice")))
            textobject.moveCursor(0, 5)
            textobject.setFont(self.font_regular, 10)
            textobject.textLine(self._normalize(self.invoice.refers.number))
        else:
            textobject.textLine(self._normalize(self._upper("Invoice number")))
            textobject.moveCursor(0, 5)
            textobject.setFont(self.font_regular, 10)
            textobject.textLine(self._normalize(self.invoice.number))
        textobject.moveCursor(0, 5)
        ...
        if self.invoice.is_cancellation:
            textobject.setFont(self.font_bold, 8)
            textobject.textLine(self._normalize(self._upper("Cancellation date")))
            textobject.moveCursor(0, 5)
            textobject.setFont(self.font_regular, 10)
            textobject.textLine(self._normalize(date_format(self.invoice.date, "DATE_FORMAT")))
            textobject.moveCursor(0, 5)
            textobject.setFont(self.font_bold, 8)
            textobject.textLine(self._normalize(self._upper("Original invoice date")))
            textobject.moveCursor(0, 5)
            textobject.setFont(self.font_regular, 10)
            ...
            textobject.setFont(self.font_bold, 8)
            textobject.textLine(self._normalize(self._upper("Invoice date")))
            textobject.moveCursor(0, 5)
            textobject.setFont(self.font_regular, 10)
            textobject.textLine(self._normalize(date_format(self.invoice.date, "DATE_FORMAT")))
            textobject.moveCursor(0, 5)
    ...
    def _draw_event_label(self, canvas):
        ...
        textobject.setFont(self.font_bold, 8)
        textobject.textLine(self._normalize(self._upper("Event")))
        canvas.drawText(textobject)
    ...
    def _draw_event(self, canvas):
        ...
        if not self.invoice.event.has_subevents and self.invoice.event.settings.show_dates_on_frontpage:
            ...
            show_end_date = (
                self.invoice.event.settings.show_date_to
                and self.invoice.event.date_to
                and self.invoice.event.date_to.astimezone(tz).date()
                != self.invoice.event.date_from.astimezone(tz).date()
            )
            if show_end_date:
                p_str = (
    ...
    def _draw_testmode(self, canvas):
        if self.invoice.order.testmode:
            ...
            canvas.drawRightString(self.pagesize[0] - 20 * mm, (297 - 100) * mm, self._normalize("TEST MODE"))
            canvas.restoreState()

    def _on_first_page(self, canvas: Canvas, doc):
        canvas.setCreator("pretix.eu")
        canvas.setTitle("Invoice {num}").format(num=self.invoice.number)
    ...
    def _get_intro(self):
        ...
        if self.invoice.internal_reference:
            story.append(
                Paragraph(
                    self._normalize(
                        "Customer reference: {reference}".format(
        ...
        if self.invoice.invoice_to_vat_id:
            story.append(
                Paragraph(
                    self._normalize("Customer VAT ID") + ": " + self._clean_text(self.invoice.invoice_to_vat_id),
                    self.stylesheet["Normal"],
                )
            )
        ...
        if self.invoice.invoice_to_beneficiary:
            story.append(
                Paragraph(
                    self._normalize("Beneficiary") + ":<br />" + self._clean_text(self.invoice.invoice_to_beneficiary),
                    self.stylesheet["Normal"],
                )
            )
        ...
        if self.invoice.introductory_text:
            story.append(
                Paragraph(self._clean_text(self.invoice.introductory_text, tags=["br"]), self.stylesheet["Normal"])
            )
    ...
    def _get_story(self, doc):
        ...
        story = [
            ...
            Paragraph(
                self._normalize("Tax Invoice" if str(self.invoice.invoice_from_country) == "AU" else "Invoice")
                if not self.invoice.is_cancellation
                else self._normalize("Cancellation"),
        ...
        if has_taxes:
            tdata = [
                (
                    Paragraph(self._normalize("Description"), self.stylesheet["Bold"]),
                    Paragraph(self._normalize("Qty"), self.stylesheet["BoldRightNoSplit"]),
                    Paragraph(self._normalize("Tax rate"), self.stylesheet["BoldRightNoSplit"]),
                    Paragraph(self._normalize("Net"), self.stylesheet["BoldRightNoSplit"]),
                    Paragraph(self._normalize("Gross"), self.stylesheet["BoldRightNoSplit"]),
                )
            ]
        else:
            tdata = [
                (
                    Paragraph(self._normalize("Description"), self.stylesheet["Bold"]),
                    Paragraph(self._normalize("Qty"), self.stylesheet["BoldRightNoSplit"]),
                    Paragraph(self._normalize("Amount"), self.stylesheet["BoldRightNoSplit"]),
                )
            ]
        ...
        for (description, tax_rate, tax_name, net_value, gross_value, *ignored), lines in addon_aware_groupby(
            ...
            if has_taxes:
                if len(lines) > 1:
                    single_price_line = "Single price: {net_price} net / {gross_price} gross".format(
                ...
                tdata.append(
                    (
                        Paragraph(self._clean_text(description, tags=["br"]), self.stylesheet["Normal"]),
                        str(len(lines)),
                        localize(tax_rate) + " %",
                        Paragraph(
                ...
                if len(lines) > 1:
                    single_price_line = "Single price: {price}".format(
        ...
        if has_taxes:
            tdata.append(
                [
                    Paragraph(self._normalize("Invoice total"), self.stylesheet["Bold"]),
                    "",
                    "",
                    "",
            ...
            tdata.append(
                [
                    Paragraph(self._normalize("invoice", "Invoice total"), self.stylesheet["Bold"]),
                    "",
                    money_filter(total, self.invoice.event.currency),
                ]
            )
            colwidths = [a * doc.width for a in (0.65, 0.20, 0.15)]
        ...
        if not self.invoice.is_cancellation:
            if self.invoice.event.settings.invoice_show_payments and self.invoice.order.status == Order.STATUS_PENDING:
                ...
                if pending_sum != total:
                    tdata.append(
                        [Paragraph(self._normalize("Received payments"), self.stylesheet["Normal"])]
                        + (["", "", ""] if has_taxes else [""])
                        + [money_filter(pending_sum - total, self.invoice.event.currency)]
                    )
                    tdata.append(
                        [Paragraph(self._normalize("Outstanding payments"), self.stylesheet["Bold"])]
                        + (["", "", ""] if has_taxes else [""])
                ...
                and self.invoice.order.payments.filter(
                ...
                tdata.append(
                    [Paragraph(self._normalize("Paid by gift card"), self.stylesheet["Normal"])]
                    + (["", "", ""] if has_taxes else [""])
                    + [money_filter(giftcard_sum, self.invoice.event.currency)]
                )
                tdata.append(
                    [Paragraph(self._normalize("Remaining amount"), self.stylesheet["Bold"])]
                    + (["", "", ""] if has_taxes else [""])
        ...
        if self.invoice.payment_provider_text:
            story.append(Paragraph(self._normalize(self.invoice.payment_provider_text), self.stylesheet["Normal"]))
        ...
        if self.invoice.additional_text:
            story.append(
                Paragraph(self._clean_text(self.invoice.additional_text, tags=["br"]), self.stylesheet["Normal"])
            )
        ...
        thead = [
            Paragraph(self._normalize("Tax rate"), self.stylesheet["Fineprint"]),
            Paragraph(self._normalize("Net value"), self.stylesheet["FineprintRight"]),
            Paragraph(self._normalize("Gross value"), self.stylesheet["FineprintRight"]),
            Paragraph(self._normalize("Tax"), self.stylesheet["FineprintRight"]),
            "",
        ]
        tdata = [thead]
        ...
        if any(rate != 0 and gross != 0 for (rate, name), gross in grossvalue_map.items()) and has_taxes:
            ...
            story.append(
                KeepTogether(
                    [
                        Paragraph(self._normalize("Included taxes"), self.stylesheet["FineprintHeading"]),
                        table,
                    ]
                )
            )
                        ...
                        [
                            ...
                            Paragraph(
                                self._normalize(
                                    ...
                                    "{date}, this corresponds to:".format(
...
class Modern1Renderer(ClassicInvoiceRenderer):
    identifier = "modern1"
    verbose_name = "Default invoice renderer (European-style letter)"
    ...
    def _draw_invoice_from(self, canvas):
        if not self.invoice.invoice_from:
            return
        c = [self._clean_text(l) for l in self.invoice.address_invoice_from.strip().split("\n")]
    ...
    def _draw_metadata(self, canvas):
        ...
        while value_size >= 5:
            if self.event.settings.invoice_renderer_highlight_order_code:
                kwargs = dict(bold=True, sublabel="(Please quote at all times.)")
            else:
                kwargs = {}
            objects = [
                _draw(
                    "Order code",
                    self.invoice.order.full_code,
                    value_size,
                    self.left_margin,
                    45 * mm,
                    **kwargs,
            ...
            p = Paragraph(
                ...
                style=ParagraphStyle(
                    name=f"Normal{value_size}",
                    fontName=self.font_regular,
                    fontSize=value_size,
                    leading=value_size * 1.2,
                ),
            )
            ...
            if self.invoice.is_cancellation:
                objects += [
                    _draw(
                        ...
                        self.invoice.number,
                        value_size,
                        self.left_margin + 50 * mm,
                        45 * mm,
                    ),
                    _draw(
                        "Original invoice",
                        self.invoice.refers.number,
                        value_size,
                        self.left_margin + 100 * mm,
                        date_x - self.left_margin - 100 * mm - 5 * mm,
                    ),
                ]
            else:
                objects += [
                    _draw(
                        ...
                        self.invoice.number,
                        value_size,
                        self.left_margin + 70 * mm,
                        date_x - self.left_margin - 70 * mm - 5 * mm,
                    ),
                ]
        ...
        textobject.setFont(self.font_regular, 8)
        if self.invoice.is_cancellation:
            textobject.textLine(self._normalize("Cancellation date"))
        else:
            textobject.textLine(self._normalize("Invoice date"))
        canvas.drawText(textobject)


class Modern1SimplifiedRenderer(Modern1Renderer):
    identifier = "modern1simplified"
    verbose_name = "Simplified invoice renderer"
