...
-- user_tag: same printed PIN may exist on multiple chips (distinct uid). At most one row per PIN
-- may have uid IS NULL in the tree scope. UID remains unique when set (uid_is_unique below).
...
alter table user_tag drop constraint if exists uid_is_unique;
