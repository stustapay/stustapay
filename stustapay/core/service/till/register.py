...
from stustapay.core.schema.account import Account, ACCOUNT_CASH_VAULT
...
class TillRegisterService(DBService):
    ...
    async def list_cash_register_stockings_admin(self, ctx: UserContext) -> list[CashRegisterStocking]:
        return await self._list_cash_register_stockings(conn=ctx.conn)

    @with_db_transaction
    @requires_terminal()
    async def list_cash_register_stockings_terminal(self, ctx: UserContext) -> list[CashRegisterStocking]:
        return await self._list_cash_register_stockings(conn=ctx.conn)

    @with_db_transaction
    @requires_user([Privilege.till_management])
    async def create_cash_register_stockings(
        self, ctx: UserContext, *, stocking: NewCashRegisterStocking
    ) -> CashRegisterStocking:
        stocking_id = await ctx.conn.fetchval(
        ...
        updated = await self._get_cash_register_stocking(conn=ctx.conn, stocking_id=stocking_id)
    ...
    async def update_cash_register_stockings(
        self, ctx: UserContext, *, stocking_id: int, stocking: NewCashRegisterStocking
    ) -> CashRegisterStocking:
        stocking_id = await ctx.conn.fetchval(
    ...
    @with_db_transaction
    @requires_user([Privilege.till_management])
    async def delete_cash_register_stockings(self, ctx: UserContext, *, stocking_id: int):
    ...
    async def list_cash_registers_terminal(
        self, ctx: UserContext, *, hide_assigned_registers=False
    ) -> list[CashRegister]:
        return await self._list_cash_registers(conn=ctx.conn, hide_assigned_registers=hide_assigned_registers)

    @with_db_transaction
    @requires_user([Privilege.till_management])
    async def list_cash_registers_admin(self, ctx: UserContext, *, hide_assigned_registers=False) -> list[CashRegister]:
        return await self._list_cash_registers(conn=ctx.conn, hide_assigned_registers=hide_assigned_registers)

    @with_db_transaction
    @requires_user([Privilege.till_management])
    async def create_cash_register(self, ctx: UserContext, *, new_register: NewCashRegister) -> CashRegister:
        return await create_cash_register(conn=ctx.conn, new_register=new_register)

    @with_db_transaction
    @requires_user([Privilege.till_management])
    async def update_cash_register(
        self, ctx: UserContext, *, register_id: int, register: NewCashRegister
    ) -> CashRegister:
        ...
        r = await get_cash_register(conn=ctx.conn, register_id=register_id)
        assert r is not None
        return r

    @with_db_transaction
    @requires_user([Privilege.till_management])
    async def delete_cash_register(self, ctx: UserContext, *, register_id: int):
        result = await ctx.conn.execute(
    ...
    async def stock_up_cash_register(
        ...
        *,
        ...
        register_stocking = await self._get_cash_register_stocking(conn=ctx.conn, stocking_id=stocking_id)
        ...
        user_id = row["id"]
        is_logged_in = await ctx.conn.fetchval(
        ...
        await ctx.conn.fetchval("update usr set cash_register_id = $1 where id = $2", cash_register_id, user_id)
        ...
        await book_transaction(
            ...
            conducting_user_id=ctx.current_user.id,
        )
        return True
    ...
    async def modify_cashier_account_balance(
        ...
        *,
        ...
        assert ctx.current_user.transport_account_id is not None
        transport_account = await get_account_by_id(conn=ctx.conn, account_id=ctx.current_user.transport_account_id)
        ...
        await book_money_transfer(
            conn=ctx.conn,
            originating_user_id=ctx.current_user.id,
            till_id=row["till_id"],
            bookings=bookings,
    ...
    @with_retryable_db_transaction()
    @requires_terminal([Privilege.cashier_management])
    async def modify_transport_account_balance(self, ctx: UserContext, *, orga_tag_uid: int, amount: float):
        transport_account = await get_transport_account_by_tag_uid(conn=ctx.conn, orga_tag_uid=orga_tag_uid)
    ...
    async def transfer_cash_register_admin(
        self, ctx: UserContext, *, source_cashier_id: int, target_cashier_id: int
    ) -> CashRegister:
        return await self._transfer_cash_register(
            conn=ctx.conn, source_cashier_id=source_cashier_id, target_cashier_id=target_cashier_id
        )

    @with_retryable_db_transaction()
    @requires_terminal()
    async def transfer_cash_register_terminal(
        self, ctx: UserContext, *, source_cashier_tag_uid: int, target_cashier_tag_uid: int
    ):
        source_cashier_id = await ctx.conn.fetchval(
            "select id from usr where user_tag_uid = $1", source_cashier_tag_uid
        )
        target_cashier_id = await ctx.conn.fetchval(
            "select id from usr where user_tag_uid = $1", target_cashier_tag_uid
        )
        return await self._transfer_cash_register(
            conn=ctx.conn, source_cashier_id=source_cashier_id, target_cashier_id=target_cashier_id
        )
