...
class ProductService(DBService):
    ...
    async def create_product(self, *, conn: asyncpg.Connection, product: NewProduct) -> Product:
        row = await conn.fetchrow(
            "insert into product (name, price, tax_name, target_account_id) values ($1, $2, $3, $4) "
            "returning id, name, price, tax_name, target_account_id",
            product.name,
            product.price,
            product.tax_name,
    ...
    async def update_product(
        ...
        row = await conn.fetchrow(
            "update product set name = $2, price = $3, tax_name = $4, target_account_id = $5 where id = $1 "
            "returning id, name, price, tax_name, target_account_id",
            product_id,
            product.name,
            product.price,
