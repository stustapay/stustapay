...
class AccountService(DBService):
    ...
    async def get_user_tag_detail(self, ctx: UserContext, *, user_tag_uid: int) -> Optional[UserTagDetail]:
        row = await ctx.conn.fetchrow("select * from user_tag_with_history utwh where user_tag_uid = $1", user_tag_uid)
        if row is None:
            return None
    ...
    @with_db_transaction
    @requires_user([Privilege.account_management])
    async def update_user_tag_comment(self, ctx: UserContext, *, user_tag_uid: int, comment: str) -> UserTagDetail:
        ret = await ctx.conn.fetchval(
        ...
        detail = await self.get_user_tag_detail(ctx, user_tag_uid=user_tag_uid)
        assert detail is not None
        return detail

    @with_db_transaction
    @requires_user([Privilege.account_management])
    async def list_system_accounts(self, ctx: UserContext) -> list[Account]:
        cursor = ctx.conn.cursor("select * from account_with_history where type != 'private'")
        result = []
    ...
    @with_db_transaction
    @requires_user([Privilege.account_management])
    async def get_account(self, ctx: UserContext, *, account_id: int) -> Account:
        account = await get_account_by_id(conn=ctx.conn, account_id=account_id)
        if account is None:
    ...
    @with_db_transaction
    @requires_user([Privilege.account_management])
    async def get_account_by_tag_uid(self, ctx: UserContext, *, user_tag_uid: int) -> Optional[Account]:
        return await get_account_by_tag_uid(conn=ctx.conn, tag_uid=user_tag_uid)

    @with_db_transaction
    @requires_user([Privilege.account_management])
    async def find_accounts(self, ctx: UserContext, *, search_term: str) -> list[Account]:
        value_as_int = None
    ...
    @with_db_transaction
    @requires_user([Privilege.account_management])
    async def disable_account(self, ctx: UserContext, *, account_id: int):
        row = await ctx.conn.fetchval("update account set user_tag_uid = null where id = $1 returning id", account_id)
        if row is None:
            raise NotFound(element_typ="account", element_id=str(account_id))

    @with_db_transaction
    @requires_user([Privilege.account_management])
    async def update_account_balance(self, ctx: UserContext, *, account_id: int, new_balance: float) -> bool:
        raise RuntimeError("currently disallowed")
        # account = await self.get_account(ctx, account_id=account_id)
        # if account is None:
        #     return False
        #
    ...
    @with_db_transaction
    @requires_user([Privilege.account_management])
    async def update_account_vouchers(self, ctx: UserContext, *, account_id: int, new_voucher_amount: int) -> bool:
        account = await self.get_account(ctx, account_id=account_id)
        if account is None:
            return False
    ...
    async def grant_vouchers(self, ctx: UserContext, *, user_tag_uid: int, vouchers: int) -> Account:
        if vouchers <= 0:
            raise InvalidArgument("voucher amount must be positive")

        account = await get_account_by_tag_uid(conn=ctx.conn, tag_uid=user_tag_uid)
        if account is None:
    ...
    async def grant_free_tickets(self, ctx: UserContext, *, new_free_ticket_grant: NewFreeTicketGrant) -> Account:
        user_tag = await ctx.conn.fetchrow(
        ...
        account_id = await ctx.conn.fetchval(
    ...
    @with_db_transaction
    @requires_user([Privilege.account_management])
    async def update_account_comment(self, ctx: UserContext, *, account_id: int, comment: str) -> Account:
        ret = await ctx.conn.fetchval("update account set comment = $1 where id = $2 returning id", comment, account_id)
        if ret is None:
            raise NotFound(element_typ="account", element_id=account_id)

        acc = await get_account_by_id(conn=ctx.conn, account_id=account_id)
    ...
    async def switch_account_tag_uid_admin(
        self, ctx: UserContext, *, account_id: int, new_user_tag_uid: int, comment: Optional[str]
    ):
        await self._switch_account_tag_uid(
            conn=ctx.conn, account_id=account_id, new_user_tag_uid=new_user_tag_uid, comment=comment
        )

    @with_db_transaction
    @requires_terminal([Privilege.account_management])
    async def switch_account_tag_uid_terminal(
        self, ctx: TerminalContext, *, account_id: int, new_user_tag_uid: int, comment: Optional[str]
    ):
        await self._switch_account_tag_uid(
            conn=ctx.conn, account_id=account_id, new_user_tag_uid=new_user_tag_uid, comment=comment
        )
