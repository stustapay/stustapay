...
async def fetch_node(conn: Connection, node_id: int) -> Node | None:
    node = await conn.fetch_maybe_one(
        Node,
        "select n.*, '{}'::json array as children from node_with_allowed_objects n where n.id = $1",
        node_id,
    )
    if node is None:
        return None
...
async def fetch_event_design(conn: Connection, node_id: int) -> EventDesign:
    ...
    if design is None:
        return EventDesign(
            ...
            app_logo_blob_id=None,
            customer_logo_blob_id=None,
            wristband_guide_blob_id=None,
        )
    return design
