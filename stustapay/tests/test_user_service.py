...
class UserServiceTest(BaseTestCase):
    ...
    async def test_user_creation(self):
        cashier = await self.user_service.create_cashier(
            token=self.terminal_token, new_user=NewUser(login="test-cashier", user_tag_uid=self.cashier_uid)
        )
        ...
        finanzorga = await self.user_service.create_finanzorga(
            token=self.terminal_token, new_user=NewUser(login="test-finanzorga", user_tag_uid=self.finanzorga_uid)
        )
