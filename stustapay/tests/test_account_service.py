...
async def test_switch_user_tag(
    ...
    acc = await account_service.get_account(token=admin_token, node_id=event_node.id, account_id=account_id)
    ...
    user_info = await user_service.get_user(token=admin_token, node_id=event_node.id, user_id=user.id)
    assert user_info is not None
    assert new_user_tag.uid == user_info.user_tag_uid

    user_tag_2 = await user_tag_service.get_user_tag_detail(
        token=admin_token, node_id=event_node.id, user_tag_uid=new_user_tag.uid
    )
...
async def test_account_comment_updates(
    ...
    await account_service.update_account_comment(
        token=admin_token, node_id=event_node.id, account_id=account_id, comment="foobar"
    )
    acc = await account_service.get_account(token=admin_token, node_id=event_node.id, account_id=account_id)
    assert acc is not None
