...
async def test_get_user_info(
    ...
    user_info = await terminal_service.get_user_info(token=terminal_token, user_tag_uid=cashier.user_tag_uid)
    assert user_info is not None

    with pytest.raises(AccessDenied):
        await terminal_service.get_user_info(token=terminal_token, user_tag_uid=event_admin_tag.uid)
    ...
    user_info = await terminal_service.get_user_info(token=terminal_token, user_tag_uid=event_admin_tag.uid)
    assert user_info is not None
