...
async def test_terminal_registration_flow(
    till_service: TillService, event_node: Node, admin_token: str, terminal_token: str, till: Till
):
    ...
    till_info = await till_service.get_till(token=admin_token, node_id=event_node.id, till_id=till.id)
    assert till_info is not None
    await till_service.register_terminal(registration_uuid=till_info.registration_uuid)
    logged_out = await till_service.logout_terminal_id(token=admin_token, node_id=event_node.id, till_id=till.id)
    assert logged_out
