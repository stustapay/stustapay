...
@router.get(
    ...
    response_model=list[OrderWithBon],
)
...
@router.get(
    ...
    response_model=CustomerPortalApiConfig,
)
...
@router.get(
    "/event-design",
    summary="get event design for the configured node",
    response_model=EventDesign,
)
async def get_event_design(
    customer_service: ContextCustomerService,
    base_url: str,
):
    return await customer_service.get_event_design(base_url=base_url)
