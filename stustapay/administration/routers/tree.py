...
@router.post("/events/{node_id}/event-design/app-logo")
async def update_app_logo(
    ...
    await tree_service.update_app_logo(token=token, node_id=node_id, image=payload)


@router.post("/events/{node_id}/event-design/customer-logo")
async def update_customer_logo(
    token: CurrentAuthToken,
    tree_service: ContextTreeService,
    node_id: int,
    payload: NewBlob,
):
    await tree_service.update_customer_logo(token=token, node_id=node_id, image=payload)


@router.post("/events/{node_id}/event-design/wristband-guide")
