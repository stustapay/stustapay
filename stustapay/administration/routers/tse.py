...
async def list_tses(token: CurrentAuthToken, tse_service: ContextTseService, node_id: Optional[int] = None):
    return normalize_list(await tse_service.list_tses(token=token, node_id=node_id))


@router.post("/", response_model=Tse)
async def create_tse(
    token: CurrentAuthToken, tse_service: ContextTseService, new_tse: NewTse, node_id: Optional[int] = None
):
