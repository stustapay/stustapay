...
async def list_tax_rates(token: CurrentAuthToken, tax_service: ContextTaxRateService, node_id: Optional[int] = None):
    return normalize_list(await tax_service.list_tax_rates(token=token, node_id=node_id), primary_key="name")


@router.post("", response_model=TaxRate)
async def create_tax_rate(
    tax_rate: TaxRate, token: CurrentAuthToken, tax_service: ContextTaxRateService, node_id: Optional[int] = None
):
...
async def get_tax_rate(
    tax_rate_name: str, token: CurrentAuthToken, tax_service: ContextTaxRateService, node_id: Optional[int] = None
):
    tax_rate = await tax_service.get_tax_rate(token=token, tax_rate_name=tax_rate_name, node_id=node_id)
    if tax_rate is None:
...
async def update_tax_rate(
    ...
    tax_rate = await tax_service.update_tax_rate(
        token=token, tax_rate_name=tax_rate_name, tax_rate=tax_rate, node_id=node_id
    )
    if tax_rate is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
...
async def delete_tax_rate(
    ...
    deleted = await tax_service.delete_tax_rate(token=token, tax_rate_name=tax_rate_name, node_id=node_id)
    if not deleted:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
