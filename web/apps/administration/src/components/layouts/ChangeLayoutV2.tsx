...
export interface ChangeLayoutV2Props<T extends Record<string, any>> {
  title: string;
  submitLabel: string;
...
export function ChangeLayoutV2<T extends Record<string, any>>({
  title,
  successRoute,
  submitLabel,
  saveAndClearLabel,
  ...
  const handleSubmit = (values: T, { setSubmitting, resetForm }: FormikHelpers<T>) => {
    ...
    const transaction = onSubmit(submittingValues);
    transaction.isPersisted.promise
      .then((transaction) => {
        ...
        if (isAddAnother) {
          ...
          navigate(typeof successRoute === "function" ? successRoute(transaction.mutations[0].modified) : successRoute);
        }
      })
      .catch((transaction) => {
        setSubmitting(false);
        console.warn("error in change", transaction.error?.message);
      });
  };
