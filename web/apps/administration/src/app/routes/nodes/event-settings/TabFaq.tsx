...
export const TabFaq: React.FC<{ nodeId: number; eventSettings: RestrictedEventSettings }> = ({
  nodeId,
  eventSettings,
}) => {
        ...
        <Form onSubmit={formik.handleSubmit}>
          <Stack spacing={2}>
            ...
            <FormControl error={!!formik.errors.translation_texts}>
              <MarkdownEditor
                label={t("settings.faq.content")}
                value={formik.values.translation_texts[language]?.["faq"] ?? ""}
                onChange={(val) => {
                  const newSettings = updateTranslationTexts(formik.values.translation_texts, language, "faq", val);
