...
export const TabImpressum: React.FC<{ nodeId: number; eventSettings: RestrictedEventSettings }> = ({
  nodeId,
  eventSettings,
}) => {
      ...
      {(formik) => (
        <Form onSubmit={formik.handleSubmit}>
          <Stack spacing={2}>
            ...
            <FormGroup>
              <FormControlLabel
                label={t("settings.impressum.preview")}
            ...
            <FormControl error={!!formik.errors.translation_texts}>
              <MarkdownEditor
                label={t("settings.impressum.content")}
                value={formik.values.translation_texts[language]?.["impressum"] ?? ""}
                onChange={(val) => {
                  const newSettings = updateTranslationTexts(formik.values.translation_texts, language, "impressum", val);
