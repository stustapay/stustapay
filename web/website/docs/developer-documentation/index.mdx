...
You should now have the `stustapay` command available in the virtualenv `.venv`.
...
```shell
uv run stustapay -c etc/config.devel.yaml db migrate
```
...
See [Customer Portal E2E Tests](customerportal-e2e.md) for setup and prerequisites.
